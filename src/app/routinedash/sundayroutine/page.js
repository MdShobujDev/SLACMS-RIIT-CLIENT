    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await axios.put(
          `https://slacms-riit-server-ashen.vercel.app/api/routines/928d0f8b-3f29-4180-bfef-366571110ce9/weeklyRoutine/sunday`,
          Newperiods
        );
        ...
        if (response.data.statusText === "ok") {
          ...
          nextStep();
          router.push("/routinedash/mondayroutine");
        }
      } catch (error) {
    ...
    <div className="w-full mb-5 mx-auto">
      ...
      <div className="py-6">
        <h1 className="font-bold text-3xl">
          Step : 1 - Set here your{" "}
          <span className="text-red-600 text-4xl font-extrabold">Sunday</span>{" "}
          class routine
        </h1>
            ...
            <div className="grid grid-cols-1 gap-5">
              {periods.map((period, index) => (
                <div key={index} className="grid grid-cols-1 gap-5 items-end">
                  <h1 className="text-xl text-red-600  py-8 font-bold">
                    Period - {index + 1}
                  </h1>
                  <div>
                    <label
